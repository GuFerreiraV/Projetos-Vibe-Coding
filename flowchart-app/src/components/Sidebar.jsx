...
import { Square, Circle, Triangle, Diamond, Type } from 'lucide-react';
    ...
    return (
        <aside className="sidebar">
            ...
            <div className="dndnode dndnode-default" onDragStart={(event) => onDragStart(event, 'process', 'Process')} draggable>
                <Square size={20} className="text-indigo-500" />
                Process
            </div>

            <div className="dndnode" onDragStart={(event) => onDragStart(event, 'decision', 'Decision')} draggable>
                <Diamond size={20} className="text-yellow-500" />
                Decision
            </div>

            <div className="dndnode" onDragStart={(event) => onDragStart(event, 'inputOutput', 'Input/Output')} draggable>
                <div style={{ width: 20, height: 16, transform: 'skew(-20deg)', border: '2px solid #22c55e', borderRadius: 2 }}></div>
                Input/Output
            </div>

            <div className="dndnode" onDragStart={(event) => onDragStart(event, 'manualInput', 'Manual Input')} draggable>
                <div style={{ width: 20, height: 16, clipPath: 'polygon(0 20%, 100% 0, 100% 100%, 0 100%)', background: '#6366f1' }}></div>
                Manual Input
            </div>

            <div className="dndnode" onDragStart={(event) => onDragStart(event, 'database', 'Database')} draggable>
                <div style={{ width: 16, height: 20, borderRadius: '50% / 15%', border: '2px solid #a855f7' }}></div>
                Database
            </div>

            <div className="dndnode" onDragStart={(event) => onDragStart(event, 'subprocess', 'Subprocess')} draggable>
